...
# Backup file is encrypted with AES-256-CBC with sha256 message-digest (option)
...
# openssl enc -aes-256-cbc -pbkdf2 -iter 100000 -md sha256 -in [encrypted backup] -out decrypted_backup.tgz -pass pass:[backup password] -d
...
start_backup() {
    ...
    if ${ENCRYPTFLG}; then
        ...
        openssl enc -aes-256-cbc -pbkdf2 -iter 100000 -md sha256 -in "${TARFILE}" -out "${ENC_TARFILE}" -pass pass:"${BACKUPPASS}"
...
rclone_upload() {
    if ${RCLONE_FLG} && ${RCLONE_COMMAND}; then
        ...
        if [[ -n "${RCLONE_FOLDER}" ]]; then
            if ! rclone ls "${RCLONE_NAME}:${RCLONE_FOLDER}" > /dev/null 2>&1; then
        ...
        if ! rclone copy "${OUT_FILE}" "${RCLONE_NAME}:${RCLONE_FOLDER}" >> "${LOGFILE}"; then
...
ftp_upload() {
    if ${FTP_FLG}; then
        ...
        local FTP_OUT_FILE
        FTP_OUT_FILE=$(basename "${OUT_FILE}")
...
delete_gdrive_file() {
    ...
    if ${DELETE_REMOTE_FILE_FLG} && ${RCLONE_COMMAND}; then
        if rclone ls "${RCLONE_NAME}:${RCLONE_FOLDER}/${FILENAME}" > /dev/null 2>&1; then
            if rclone delete "${RCLONE_NAME}:${RCLONE_FOLDER}/${FILENAME}" >> "${LOGFILE}"; then
